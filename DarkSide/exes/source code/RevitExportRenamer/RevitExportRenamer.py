...
class FileProcessorApp(_GUI_Base_Util.BaseGUI):
    def __init__(self, root):
        ...
        self.root.configure(bg=self.BACKGROUND_COLOR_HEX)  # Even darker grey

        # Set the window icon
        icon_path = os.path.join(os.path.dirname(__file__), "icon_logo.ico")
        ...
        if datetime.datetime.now() >= WARNING_DATE:
            self.warning_label = tk.Label(self.root, text=f"The tool will expire on {EXPIRATION_DATE.strftime('%B %d, %Y')}", bg=self.BACKGROUND_COLOR_HEX, fg='red')
    ...
    def create_widgets(self):
        ...
        self.pick_files_button = tk.Button(self.root, text="Pick Files", command=self.pick_files, bg=self.BACKGROUND_COLOR_HEX, fg='white')
        self.pick_files_button.grid(row=1, column=0, padx=10, pady=10)

        self.pick_output_folder_button = tk.Button(self.root, text="Pick Output Folder", command=self.pick_output_folder, bg=self.BACKGROUND_COLOR_HEX, fg='white')
        self.pick_output_folder_button.grid(row=1, column=1, padx=10, pady=10)

        self.prefix_label = tk.Label(self.root, text="Prefix:", bg=self.BACKGROUND_COLOR_HEX, fg='white')
        ...
        self.example_label = tk.Label(self.root, text="Example: [prefix] processedName.extension", bg=self.BACKGROUND_COLOR_HEX, fg='white')
        self.example_label.grid(row=3, column=0, columnspan=3, padx=10, pady=10)

        self.process_button = tk.Button(self.root, text="Process", command=self.process_files, bg=self.BACKGROUND_COLOR_HEX, fg='white')
        self.process_button.grid(row=4, column=0, columnspan=3, padx=10, pady=20)

        self.open_output_folder_button = tk.Button(self.root, text="Open Output Folder", command=self.open_output_folder, bg=self.BACKGROUND_COLOR_HEX, fg='white', state=tk.DISABLED)
