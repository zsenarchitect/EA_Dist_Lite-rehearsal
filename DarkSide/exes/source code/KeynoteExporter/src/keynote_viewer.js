...
// Notes suffix char comes from config via data-notes-suffix-codepoint on the button (e.g. 200A = U+200A HAIR SPACE)
function getNotesChar() {
  const btn = document.getElementById('copy-notes-char-btn');
  const hex = btn && btn.getAttribute('data-notes-suffix-codepoint');
  if (hex) {
...
function copyNotesChar() {
  const btn = document.getElementById('copy-notes-char-btn');
  const notesChar = getNotesChar();
  const originalText = btn ? btn.textContent : '';
  function revert() {
    if (btn) btn.textContent = originalText;
  }
  if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard.writeText(notesChar).then(function() {
...
function fallbackCopy() {
  const notesChar = getNotesChar();
  const hex = notesChar.charCodeAt(0).toString(16).toUpperCase();
  const input = document.createElement('input');
  input.value = notesChar;
  ...
  try {
    if (document.execCommand('copy')) {
      ...
      alert('Copy failed. Character: U+' + hex + '. Paste from a Unicode source if needed.');
    }
  } catch (e) {
    alert('Copy failed. Character: U+' + hex + '. Paste from a Unicode source if needed.');
  }
