"""
KeynoteParser - Excel Data Parsing Module

This module handles all Excel file parsing and KeynoteData object creation.
It reads Excel files, identifies headers, and creates structured data objects.

Header matching rule (important):
    Columns are identified by their HEADER TEXT, not by their position/index.
    - The header row is whichever row first contains ANY known column name
      (from keynote_config.yaml: string_columns + interior/exterior_scope_columns).
    - All downstream lookups are keyed by the exact header string, so columns may
      sit in any order and extra/unknown columns are simply ignored.
    - Matching is exact after .strip(): case, dots, and spacing must match the
      YAML names verbatim. "INT. WALL" matches; "INT.WALL" or "Int. Wall" will be
      silently dropped (no missing-column warning is raised).
    - The header must live on a single row. A two-row / merged header will not
      be recognized.
"""
