...
def combined_image_text(icon_list, size):
    ...
    for i, image_handler in enumerate(icon_list):
        ...
        # Check if the resized image fits within the combined image
        if resized_image.size[0] > combined_width or resized_image.size[1] > combined_height:
            print(f"Warning: Image {i} exceeds combined image dimensions. Skipping.")
            raise Exception("Image exceeds combined image dimensions for {}".format(image_handler.path))
...
if __name__ == "__main__":
    ...
    sample_icon = IconHandler(r"C:\Users\szhang\design-repo\EnneadTab-OS\DarkSide\RuiWriter\default_icon.png")
    # sample_icon.get_bitmap_text(16)
    print (IconHandler.chain_bitmap_text([sample_icon]))